...
class G:
    ...
    traceSheet = traceFile.add_sheet(
        'sheet '+str(sheetIndex), cell_overwrite_ok=True)  # create excel sheet
...
def setWIP(entityList):
    ...
    for entity in entityList:
        ...
        # XXX Orders do no more run in the system, instead we have OrderDesigns
            ...
            currentObjectIds = entity.remainingRoute[0].get(
                'stationIdsList', [])
            ...
            entity.currentStep = entity.remainingRoute.pop(0)
            entity.schedule.append({"station": object,
                                    "entranceTime": G.env.now})  # append the time to schedule so that it can be read in the result
                ...
                if stationClass in ['Machine', 'BatchScrapMachine', 'MachineJobShop', 'BatchDecomposition', 'BatchReassembly', 'M3', 'MouldAssembly',
                    ...
                    if not (entity.currentStation.initialWIP.triggered):
                        if entity.currentStation.expectedSignals['initialWIP']:
                            succeedTuple = (G.env, G.env.now)
                            entity.currentStation.initialWIP.succeed(
                                succeedTuple)
