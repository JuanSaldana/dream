...
import simpy
from dream.simulation.OperatorRouter import Router
from dream.simulation.opAss_LPmethod import opAss_LP
        ...
        while 1:
            ...
            if self.allocation:
                ...
                for station in self.availableStations:
                    station.wip = 1 + \
                        (len(station.getActiveObjectQueue())/station.capacity)
                    for predecessor in station.previous:
                        try:
                            station.wip += float(
                                len(predecessor.getActiveObjectQueue())/float(predecessor.capacity))
                        except:
                ...
                for station in self.availableStations:
                    ...
                    if maxLastAssignment:
                        lastAssignmentTime = 1.0 - \
                            float(lastAssignmentTime/float(maxLastAssignment))
                            ...
                            if machine.isBlocked or reassemblyBlocksMachine:
                                ...
                                if "Clearance" in nextObjectClassName:
                                    if not nextObject.getActiveObjectQueue():
                                        ...
                                        subBatchMachineHolds = machine.getActiveObjectQueue()[
                                            0]
                                        subBatchLineClearanceHolds = nextObject.getActiveObjectQueue()[
                                            0]
    ...
    def signalStation(self, station, operator):
        ...
        if station.broker.waitForOperator:
            if station.broker.expectedSignals['resourceAvailable']:
                ...
                self.printTrace(
                    'router', 'signalling broker of'+' '*50+station.id)
        ...
        # signal the queue proceeding the station
            ...
            if station.canAccept()\
                ...
                if station.expectedSignals['loadOperatorAvailable']:
                    self.sendSignal(receiver=station,
                                    signal=station.loadOperatorAvailable)
