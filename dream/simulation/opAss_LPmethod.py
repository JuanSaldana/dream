...
def opAss_LP(machineList, PBlist, PBskills, previousAssignment={}, weightFactors=[2, 1, 0, 2, 1, 1], Tool={}):
    ...
    PB_ass = LpVariable.dicts('PB', [(oper, mach) for oper in PBlist for mach in machines if machineList[mach]
                                     ['stationID'] in PBskills[oper]], 0, 1, cat=pulp.LpBinary)
    ...
    if weightFactors[0] > 0 and sumWIP > 0:
        obj.append([machineList[mach]['WIP']*PB_ass[(oper, mach)]*weightFactors[0]/float(sumWIP)
                    for oper in PBlist for mach in machines if machineList[mach]['stationID'] in PBskills[oper]])
    ...
    if weightFactors[1] > 0:
        ...
        Delta_Station = LpVariable.dicts("D_station", [(st1, st2) for i1, st1 in enumerate(
            stationGroup.keys()) for st2 in stationGroup.keys()[i1 + 1:]])
            ...
            for mach1 in stationGroup[st1]:
                for oper1 in PBlist:
                    if st1 in PBskills[oper1]:
                        tempList.append(
                            PB_ass[(oper1, mach1)]/float(maxNoMachines))
                ...
                for mach2 in stationGroup[st2]:
                    for oper2 in PBlist:
                        if st2 in PBskills[oper2]:
                            finalList.append(
                                PB_ass[(oper2, mach2)]*-1/float(maxNoMachines))
        ...
        for i1, st1 in enumerate(stationGroup.keys()):
            for st2 in stationGroup.keys()[i1+1:]:
                obj.append(
                    Delta_Station[(st1, st2)]*weightFactors[1]/float(normalisingFactorDeltaStation))
    ...
    if weightFactors[2] > 0:
        ...
        Delta_Ass = LpVariable.dicts(
            "D_Ass", [(d[0], d[1]) for d in Delta_Assignment])

        # integration of third objective
        for d in Delta_Assignment:
            obj.append(Delta_Ass[(d[0], d[1])]*(-1.0 *
                                                weightFactors[2]/(2*len(previousAssignment))))
    ...
    if weightFactors[3] > 0:
        ...
        for mach in machineList:
            if machineList[mach]['stationID'] in [0, 1, 2]:
                ...
                subline[machineList[mach]['machineID']
                        ]['WIP'] += machineList[mach]['WIP']
        ...
        if chosenSubLine:
            ...
            for station in range(3):
                ...
                mach = Tool[station][chosenSubLine].name
                for oper in PBlist:
    ...
    if LastAssignmentSum > 0 and weightFactors[4] > 0:
        obj += [machineList[mach]['lastAssignment']*PB_ass[(oper, mach)]*weightFactors[4]/float(
            LastAssignmentSum) for oper in PBlist for mach in machines if machineList[mach]['stationID'] in PBskills[oper]]

    # 6th objective: max the number of pb assigned
    if weightFactors[5] > 0:
        obj += [PB_ass[(oper, mach)]*weightFactors[5]/float(len(PBlist))
                for oper in PBlist for mach in machines if machineList[mach]['stationID'] in PBskills[oper]]
    ...
    for machine in machines:
        prob += lpSum([PB_ass[(oper, machine)]
                       for oper in PBlist if machineList[machine]['stationID'] in PBskills[oper]]) <= 1

    # constraint 2: # machines assigned to an operator <= 1
    for operator in PBlist:
        prob += lpSum([PB_ass[(operator, machine)]
                       for machine in machines if machineList[machine]['stationID'] in PBskills[operator]]) <= 1
